...
test(
    ...
    before: function () {
        shell_exec('php ' . root() . 'phpkg init --project=../../DummyProject --packages-directory=vendor');
    },
    after: function () {
        reset_dummy_project();
    }
...
{
    assert_true(file_exists(realpath(root() . '../../DummyProject/vendor')), $message);
}

function assert_simple_package_cloned($message)
{
    assert_true(
        file_exists(realpath(root() . '../../DummyProject/vendor/php-repos/simple-package'))
        && file_exists(realpath(root() . '../../DummyProject/vendor/php-repos/simple-package/phpkg.config.json'))
        && file_exists(realpath(root() . '../../DummyProject/vendor/php-repos/simple-package/README.md')),
        $message
    );
}
