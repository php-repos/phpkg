...
test(
    ...
    case: function () {
        ...
        assert_error("The GitHub token is not valid. Either, you didn't set one yet, or it is not valid. Please use the `credential` command to set a valid token.", $lines[4] . PHP_EOL);
    },
    before: function () {
        shell_exec('php ' . root() . 'phpkg init --project=TestRequirements/Fixtures/EmptyProject');
        rename(root() . 'credentials.json', root() . 'credentials.json.back');
...
test(
    ...
    case: function () {
        ...
        assert_line("Setting env credential...", $lines[1] . PHP_EOL);
        assert_line("Checking installed packages...", $lines[2] . PHP_EOL);
        assert_error("Package https://github.com/php-repos/simple-package.git is already exists.", $lines[3] . PHP_EOL);
...
function assert_output($output)
{
    ...
    assert_success("Package git@github.com:php-repos/simple-package.git has been added successfully.", $lines[8] . PHP_EOL);
}
